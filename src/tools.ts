...
import { ensureFreshToken, StravaRefreshError } from "./oauth.js";
    ...
    content: [
      {
        ...
        text: JSON.stringify({
          connected: false,
          message:
            "Strava account not connected. Open the gateway link below from a browser signed into your OpenClaw gateway to authorize.",
          connectUrl: deps.getConnectUrl(),
        }),
      },
    ],
  };
...
function createAuthStatusTool(deps: ToolDeps) {
  return {
    ...
    ownerOnly: true,
    description:
      "Check whether the user's Strava account is connected. If not connected, returns a gateway connect URL the user can open to link their account.",
