...
describe("createStravaTools", () => {
  ...
  const getConnectUrl = () => "http://localhost:18789/api/plugins/strava/oauth/start";
  ...
  it("auth_status returns not-connected with connect URL when no tokens", async () => {
    const tools = createStravaTools({ config, tokenStore, getRedirectUri, getConnectUrl });
    const authTool = tools.find((t) => t.name === "strava_auth_status")!;
    const result = await authTool.execute("test-id", {});
    const data = JSON.parse(result.content[0].text);
    expect(data.connected).toBe(false);
    expect(data.connectUrl).toBe(getConnectUrl());
    expect(data.message).toContain("gateway");
  });
