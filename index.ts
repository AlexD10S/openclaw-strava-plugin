...
const stravaPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    const callbackOrigin = tryGetOrigin(callbackUrl);
    ...
    const getGatewayOrigin = () => {
      ...
      if (bind === "custom" && customHost?.trim()) {
        return `${scheme}://${customHost.trim()}:${gatewayPort}`;
      }
      return `${scheme}://localhost:${gatewayPort}`;
    };

    const getRedirectUri = () => callbackUrl ?? `${getGatewayOrigin()}${OAUTH_CALLBACK_PATH}`;
    const getConnectUrl = () => `${getGatewayOrigin()}${OAUTH_START_PATH}`;

    // Register the 4 Strava tools.
    const tools = createStravaTools({ config, tokenStore, getRedirectUri, getConnectUrl });
